...
class InvoicesIndex extends Component
{
    ...
    public function sendInvoice(Invoice $invoice, InvoiceService $invoiceService)
    {
        if ($invoiceService->sendInvoice($invoice)) {
            $this->dispatch('notify', message: 'Facture envoyée avec succès.', type: 'success');
        } else {
            $this->dispatch('notify', message: 'Erreur lors de l\'envoi de la facture.', type: 'error');
        }
    }
    ...
    public function recordPayment(InvoiceService $invoiceService)
    {
        ...
        try {
            ...
            $this->dispatch('notify', message: 'Paiement enregistré avec succès.', type: 'success');

            $this->closePaymentModal();
        } catch (\Exception $e) {
            $this->dispatch('notify', message: 'Erreur lors de l\'enregistrement du paiement: '.$e->getMessage(), type: 'error');
        }
    }
    ...
    public function cancelInvoice(Invoice $invoice, InvoiceService $invoiceService)
    {
        if ($invoiceService->cancelInvoice($invoice, 'Annulée par l\'administrateur')) {
            $this->dispatch('notify', message: 'Facture annulée avec succès.', type: 'success');
        } else {
            $this->dispatch('notify', message: 'Erreur lors de l\'annulation de la facture.', type: 'error');
        }
    }
    ...
    public function generateMonthlyInvoices(InvoiceService $invoiceService)
    {
        try {
            ...
            $this->dispatch('notify', message: 'Erreur lors de la génération des factures: '.$e->getMessage(), type: 'error');
        }
    }
