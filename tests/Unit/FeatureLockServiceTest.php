...
class FeatureLockServiceTest extends TestCase
{
    ...
    public function test_can_get_locked_features(): void
    {
        ...
        // getLockedFeatures retourne des modèles FeatureLock complets
        $lockedFeatures = $this->service->getLockedFeatures($this->company);
    ...
    public function test_can_get_company_lock_report(): void
    {
        // Créer des verrouillages avec différents statuts
        $this->service->lockFeature($this->company, FeatureEnum::PRODUCTS_MANAGE, 'Active lock');
    ...
    public function test_cache_is_invalidated_on_lock_changes(): void
    {
        // Vérifier qu'il n'y a pas de verrouillage (met en cache)
        $this->assertFalse($this->service->isFeatureLocked($this->company, FeatureEnum::PRODUCTS_MANAGE));

        // Verrouiller la fonctionnalité
        $this->service->lockFeature($this->company, FeatureEnum::PRODUCTS_MANAGE);

        // Vérifier que le cache a été invalidé et que le nouveau statut est correct
        $this->assertTrue($this->service->isFeatureLocked($this->company, FeatureEnum::PRODUCTS_MANAGE));
    }
    ...
    public function test_can_clear_all_cache(): void
    {
        ...
        $this->service->isFeatureLocked($this->company, FeatureEnum::PRODUCTS_MANAGE);
    ...
    public function test_can_handle_custom_feature_key_gracefully(): void
    {
        // Le service accepte les clés libres en plus des enum
        $lock = $this->service->lockFeature($this->company, 'custom_feature_key');

        $this->assertInstanceOf(FeatureLock::class, $lock);
        $this->assertEquals('custom_feature_key', $lock->feature_key);
    }
    ...
    public function test_bulk_operations_handle_duplicates_gracefully(): void
    {
        ...
        $features = [FeatureEnum::PRODUCTS_MANAGE->value, FeatureEnum::SALES_CREATE->value];
        $locks = $this->service->bulkLockFeatures($this->company, $features);

        // Devrait traiter toutes les fonctionnalités (updateOrCreate gère les doublons)
